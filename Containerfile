...
RUN apt-get update && apt-get install -y \
    ca-certificates sudo unzip tar wget curl apt-utils tzdata python3 python3-pip
...
# 3. Copy this repository and fix the permissions
WORKDIR /opt/sift-on-container
COPY . .
RUN chown -R sift:sift /opt/sift-on-container
